...
def run_dcrlme_rastrigin_oi_test(num_iterations: int = 10) -> None:
    """Run DCRLME test with RastriginEnv and the LZ76 wrapper with visualization."""
    ...
    env = create(
        ...
        minval=minval,
        maxval=maxval,
        fixed_init_state=False,  # Rastrigin env doesn't support FixedInitialStateWrapper
        qdax_wrappers_kwargs=[{
            "episode_length": episode_length
        }]
    )
    
    # Note: We do NOT use OffsetRewardWrapper or ClipRewardWrapper for Rastrigin
    # because the rastrigin function naturally gives negative rewards that should not be clipped
    # The fitness values from rastrigin are highly negative (approximately -3000 to 0)
    ...
    print("Running DCRL-ME with Rastrigin OI wrapper...")
    ...
    fig1.savefig(os.path.join(plots_dir, f"dcrlm_rastrigin_metrics_{timestamp}.png"))
    plt.close(fig1)
    ...
    fig2.savefig(os.path.join(plots_dir, f"dcrlm_rastrigin_archive_{timestamp}.png"))
    plt.close(fig2)
...
def test_dcrlme_rastrigin_oi(num_iterations: int) -> None:
